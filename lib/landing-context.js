 ...
 *
 *   slug:             Site slug (from _ANALYTICS_SLUG env). Use for
...
function buildAnalyticsSnippet(slug) {
  ...
  return `<!--  Analytics --><script>(function(){var slug=${slugJson};if(!slug)return;var vid=localStorage.getItem('_vid');if(!vid){vid='xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g,function(c){var r=Math.random()*16|0;return (c==='x'?r:(r&0x3|0x8)).toString(16);});localStorage.setItem('_vid',vid);}new Image().src='https://.com/api/beacon/pixel?s='+encodeURIComponent(slug)+'&v='+encodeURIComponent(vid);})();</script>`;
}

function buildLandingContext() {
  const slug = process.env._ANALYTICS_SLUG || '';
  return {
    slug,
    theme: {},
